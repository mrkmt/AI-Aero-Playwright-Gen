    ...
    def forward(self, selector: str) -> str:
        try:
            ...
            if count == 0:
                ...
                return f"Selector '{selector}' is NOT unique. It matches {count} elements."
        except Exception as e:
            return f"Error validating selector: {str(e)}"


async def agentic_heal_selector(page: Any, failing_selector: str, description: str, object_id: Optional[str] = None) -> Optional[str]:
    """
    ...
    try:
        model = LiteLLMModel(model_id="ollama/llama3.2", api_base="http://localhost:11434", api_key="ollama")
        tools = [DomInspectorTool(page), SelectorValidatorTool(page), VisualInspectorTool(page)]
